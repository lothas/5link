...
from robotcon_msgs.msg import RobotState
...
class cb_joints(object):
	def __init__(self,args = 0):
		if args == 0:
			self.cbJointNames = ['inner_ankle', 'left_hip', 'left_ankle', 'right_hip','right_ankle']
...
class robot_state(object):
	def __init__(self,args = 0):
		if args == 0:
			...
			self._JointPos = cb_joints()
			self._JointVel = cb_joints()
			self._JointEff = cb_joints()
			self._orientation = orientation()
			#elf._angular_vel = PyKDL.Vector()
			#self._l_foot_force = 0
			#self._r_foot_force = 0
			#self._l_foot_torque = PyKDL.Vector()
			#self._r_foot_torque = PyKDL.Vector()
			#self._ZMP = {'l':PyKDL.Vector(),'r':PyKDL.Vector()}
			#self._r_reflex = {'GC':False,'EEP':False,'EAP':False,'ZMP':False}
			#self._l_reflex = {'GC':False,'EEP':False,'EAP':False,'ZMP':False}
			#self._GC = contact_reflex.contact_reflex()
			#self._vel_x_filter = control_primitives.filter(a = [1,-2.374094743709352,1.929355669091215,-0.532075368312092]
			#    ,b = [0.002898194633721,0.008694583901164,0.008694583901164,0.002898194633721])
			#self._vel_y_filter = control_primitives.filter(a = [1,-2.374094743709352,1.929355669091215,-0.532075368312092]
			#    ,b = [0.002898194633721,0.008694583901164,0.008694583901164,0.002898194633721])
			#self._vel_z_filter = control_primitives.filter(a = [1,-2.374094743709352,1.929355669091215,-0.532075368312092]
			#    ,b = [0.002898194633721,0.008694583901164,0.008694583901164,0.002898194633721])
			#self._r_contact_filter = control_primitives.filter(a = [1,-2.374094743709352,1.929355669091215,-0.532075368312092]
			#    ,b = [0.002898194633721,0.008694583901164,0.008694583901164,0.002898194633721])
			#self._l_contact_filter = control_primitives.filter(a = [1,-2.374094743709352,1.929355669091215,-0.532075368312092]
			#    ,b = [0.002898194633721,0.008694583901164,0.008694583901164,0.002898194633721])
		else:
			self._time = rospy.Time(0)
			self._JointPos = cb_joints(args)
			self._JointVel = cb_joints(args)
			self._JointEff = cb_joints(args)
			self._orientation = orientation()
